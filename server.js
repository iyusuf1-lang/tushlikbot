...
app.post('/api/order/remove-item', (req, res) => {
  const { initData, restaurant_id, item_name } = req.body || {};
  const user = validateInitData(initData);
  if (!user) return res.status(401).json({ error: 'Tasdiqlanmadi' });
  if (!item_name) return res.status(400).json({ error: 'item_name kerak' });
...
bot.command('tolov_eslatma', async (ctx) => {
  if (!isAdmin(ctx)) return ctx.reply('⛔ Bu buyruq faqat adminlar uchun.');
  ...
  await ctx.reply(`⏳ ${data.userSummaries.length} ta odamga to'lov eslatmasi yuborilmoqda...`);
  ...
  for (const u of data.userSummaries) {
    const breakdown =
      data.deliveryFee > 0
        ? `  (taom: ${u.foodTotal.toLocaleString()} so'm + dastavka: ${u.deliveryShare.toLocaleString()} so'm)\n`
        : '';
    const text =
      `🍽 Bugungi to'lovingiz\n\n` +
      `💰 Jami: ${u.finalTotal.toLocaleString()} so'm\n${breakdown}` +
      `💳 Karta: ${cardNumber}\n\n` +
...
bot.command('yordam', (ctx) => {
  ctx.reply(
    "📋 Buyruqlar:\n" +
      ...
      "/tolov_eslatma — bugun buyurtma bergan HAMMAGA shaxsiy to'lov summasi va kartani yuboradi\n" +
