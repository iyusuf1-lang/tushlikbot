...
function ordersPayload(date) {
  ...
  for (const row of rows) {
    ...
    if (!group.users[row.user_id]) {
      group.users[row.user_id] = { user_name: row.user_name, items: [], total: 0, comment: '' };
    }
...
function buildReceiptText(date) {
  const rows = store.getOrders(date);
  if (!rows.length && !store.getComments(date).length) {
  ...
  for (const row of rows) {
    ...
    if (!g.users[row.user_id]) g.users[row.user_id] = { name: row.user_name, items: [], total: 0, comment: '' };
    const lineTotal = row.price * row.qty;
  ...
  for (const c of store.getComments(date)) {
    const rid = c.restaurant_id;
    if (!byRestaurant[rid]) {
      byRestaurant[rid] = { name: c.restaurant_name, users: {}, subtotal: 0 };
    }
    const g = byRestaurant[rid];
    if (!g.users[c.user_id]) g.users[c.user_id] = { name: c.user_name, items: [], total: 0, comment: '' };
    g.users[c.user_id].comment = c.text;
  }
  ...
  for (const g of Object.values(byRestaurant)) {
    ...
    for (const u of Object.values(g.users)) {
      const itemsText = u.items.length ? u.items.join(', ') : '(taom tanlanmagan)';
      text += `  ${u.name} — ${itemsText} — ${u.total.toLocaleString()} so'm\n`;
