...
async function loadCurrentOrderNote(restaurantId) {
  ...
  try {
    ...
    document.getElementById('current-order-items').innerHTML = data.items
      .map(
        (i) => `
          <div class="current-order-item">
            <span>${escapeHtml(i.name)} ×${i.qty}</span>
            <button type="button" class="remove-item-btn" data-item="${escapeHtml(i.name)}" aria-label="O'chirish">✕</button>
          </div>`
      )
      .join('');
    box.hidden = false;
...
document.getElementById('current-order-items').addEventListener('click', async (e) => {
  const btn = e.target.closest('.remove-item-btn');
  if (!btn || !currentRestaurant) return;
  try {
    const res = await fetch('/api/order/remove-item', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ initData: getInitData(), restaurant_id: currentRestaurant.id, item_name: btn.dataset.item }),
    });
    if (!res.ok) throw new Error();
    showStatus(`"${btn.dataset.item}" buyurtmadan olib tashlandi ✓`);
